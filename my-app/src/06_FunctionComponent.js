/*
    함수형 컴포넌트
    return 값이 있는 function과 동일한 구조의 컴포넌트.

    state가 없고, 생명주기함수(constructor, render, component,,,)를 사용할수 없음
    부모컴포넌트에게 props와 context를 파라미터로 전달받아 사용하고
    render함수가 없기때문에 return만 사용하여 화면을 그려준다.
...
function FunctionComponent(props){
    ...
    /*
        useState메서드는 길이 2짜리 배열을 반환해줌. 이를 구조분해할당을 이용하여 반환된 값을 저장
    ...
    */
    ...
    /*
        useEffect는 생명주기함수중 componentDidMount와 동일한 기능을 수행함.
        return되는 html코드들이 화면에 그려진후(랜더링완료된후) 실행됨
        최초 페이지 로딩될때 한번 실행되고, setString() 함수로 state값이 변경되면 다시한번 랜더링되면서 랜더링 완료후 
        재실행된다.
    */
