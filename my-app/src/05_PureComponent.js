...
/*
    ...
    Component에서는 setState함수 호출 시 내부적으로 변경하고자하는 state의 값이 같더라도, 변경으로 간주하고
    render() 함수를 호출하였음

    PureComponent에서는 바꿀 값 자체를 실제로 비교하여 값이 동일한 경우 변경되지 않았다고 간주하고 render함수를 호출
    하지 않음

    Component와 비교하여 불필요한 화면변환을 실행하지 않으므로 페이지 성능을 향상시킬수 있다.
*/
...
class Shallow extends Component{
    ...
    /* 2. shallow-equal 테스트 */
    /* 
        컴포넌트 라이프 사이클중 변경과 관련된 메서드.
        반환값에 따라 컴포넌트 업데이트할지말지 결정한다.
        (true 반환시 render함수호출, false반환시 render함수 미호출)
    */
